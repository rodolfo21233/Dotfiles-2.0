...
PanelWindow{
    ...
    property int tracklength: Math.floor(Players.selectedPlayer.length / 60)
    property int trackseconds: Players.selectedPlayer.length % 60
    property real position: Players.selectedPlayer.position / 60
    ...
    Rectangle{
        ...
        property int prevPlayerIndex: Players.activeIndex

        Connections {
            target: Players
            function onActiveIndexChanged() {
                slideAnim.dir = Players.activeIndex > popup.prevPlayerIndex ? 1 : -1
                popup.prevPlayerIndex = Players.activeIndex
                slideAnim.restart()
            }
        }
        ...
        SequentialAnimation {
            id: slideAnim
            property int dir: 1
            PropertyAction { target: playerContent; property: "x"; value: slideAnim.dir * 40 }
            PropertyAction { target: playerContent; property: "opacity"; value: 0 }
            ParallelAnimation {
                NumberAnimation { target: playerContent; property: "x"; to: 0; duration: 280; easing.type: Easing.OutCubic }
                NumberAnimation { target: playerContent; property: "opacity"; to: 1; duration: 220 }
            }
        }
        ...
        Item {
            ...
            Text {
                font.family: "Varela"
                text: tracklength + ":" + (trackseconds < 10 ? "0" + trackseconds : trackseconds)
                anchors.left: root.right
            ...
            Text {
                font.family: "Varela"
                text: Math.floor(position) + ":" + ("0" + Math.floor((position % 1) * 60)).slice(-2) 
        ...
        Row {
            id: dotsRow
            visible: Players.playerList.length > 1
            ...
            Repeater {
                model: Players.playerList.length

                delegate: Rectangle {
                    id: dot
                    required property int index

                    width: index === Players.activeIndex ? 18 : 6
                    height: 12
                    radius: 30
                    color: index === Players.activeIndex ? Colors.md3.primary : Qt.rgba(1, 1, 1, 0.35)
                    ...
                    MouseArea {
                        anchors.margins: -6
                        anchors.fill: parent
                        onClicked: Players.selectPlayerByIndex(dot.index)
                    }
                }
            }
        }
